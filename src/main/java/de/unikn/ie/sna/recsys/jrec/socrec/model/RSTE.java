...
public class RSTE extends AbstractModel
{
    private boolean            biasTerms;
    private DMatrixDouble      U, I, tempU, tempI, cacheUpdateI;
    private DVectorDouble      W_U, W_I, tempW_U, tempW_I;
    private double             learnRate, lambdaU, lambdaI, alpha;
    private SparseMatrixDouble trusts;
    ...
    protected double score(int user, int item)
    {
        ...
        for (Entry<Integer, Double> trustee : this.trusts.get(user).entrySet())
        {
            ...
            for (int f = 0; f < this.features; f++)
            {
                dotTV += this.U.get(trustee.getKey(), f) * this.I.get(item, f);
            }
            dotT += trustee.getValue() * dotTV;
        }
        dotT *= 1 - this.alpha;

        double result = this.globalBias + dotV + dotT;
        if (this.biasTerms)
        {
    ...
    {
        // clear tempU + tempW_U + cacheUpdateI
        for (int user = 0; user < this.users; user++)
        {
            ...
            for (int f = 0; f < this.features; f++)
            {
                ...
                this.cacheUpdateI.get(user)[f] = null;
            }
        }
    ...
    public void learn(int user, int item, double rating)
    {
        ...
        {
            // determine tempU
            this.tempU.get(user)[f] += commonGradient * this.alpha * this.I.get(item, f);
            for (Entry<Integer, Double> trustee : this.trusts.get(user).entrySet())
            {
                this.tempU.get(trustee.getKey())[f] += commonGradient * (1 - this.alpha) * trustee.getValue() * this.I.get(item, f);
            }

            // determine tempI
            if (this.cacheUpdateI.get(user, f) == null)
            {
                this.cacheUpdateI.get(user)[f] = 0d;
                for (Entry<Integer, Double> trustee : this.trusts.get(user).entrySet())
                {
                    this.cacheUpdateI.get(user)[f] += trustee.getValue() * this.U.get(trustee.getKey(), f);
                }
                this.cacheUpdateI.get(user)[f] *= 1 - this.alpha;
            }
            this.tempI.get(item)[f] += commonGradient * (this.alpha * this.U.get(user, f) + this.cacheUpdateI.get(user)[f]);
        }
                     ...
                     SparseMatrixDouble trusts, SparseMatrixDouble trustedBys, double minRating, double maxRating, boolean biasTerms)
    {
        ...
        this.cacheUpdateI = new DMatrixDouble();
        this.cacheUpdateI.setSize(users, features);
    }
